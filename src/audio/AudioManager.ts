...
class AudioManagerClass {
  ...
  // Track last SFX time to prevent Tone.js timing conflicts
  ...
  playSFX(effect: SoundEffect): void {
    ...
    // Ensure start time is always strictly increasing to prevent Tone.js errors
    // Add a small offset (1ms) if this call happens at or before the last SFX time
    const minOffset = 0.001; // 1ms in seconds
    const startTime = Math.max(now, this.lastSFXTime + minOffset);
