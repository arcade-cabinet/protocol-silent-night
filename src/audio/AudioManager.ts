...
class AudioManagerClass {
  ...
  private minSFXInterval = 0.01; // Minimum 10ms between SFX calls
  ...
  playSFX(effect: SoundEffect): void {
    ...
    // Ensure minimum time between SFX calls to prevent Tone.js timing conflicts
    const timeSinceLastSFX = now - this.lastSFXTime;
    const startTime = timeSinceLastSFX < this.minSFXInterval
      ? now + (this.minSFXInterval - timeSinceLastSFX)
      : now;
