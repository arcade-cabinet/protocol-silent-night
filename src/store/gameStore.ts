...
export const useGameStore = create<GameStore>((set, get) => ({
  ...
  addKill: (points) => {
    ...
    // Use functional update to avoid race conditions with rapid consecutive calls
    let newStreak = 0;
    let streakBonus = 0;
    let newScore = 0;
    let newKills = 0;
    ...
    set((state) => {
      ...
      streakBonus = newStreak > 1 ? Math.floor(points * (newStreak - 1) * 0.25) : 0;
    ...
    if (newKills >= waveReq && (currentGameState === 'PHASE_1' || currentGameState === 'LEVEL_UP')) {
