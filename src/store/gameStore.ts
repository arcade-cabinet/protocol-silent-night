...
export const useGameStore = create<GameStore>((set, get) => ({
  ...
  addKill: (points) => {
    ...
    // Use functional update to avoid race conditions with rapid kills
    set((state) => {
    ...
    const { stats, state: currentState, runProgress } = get();
    const waveReq = CONFIG.WAVE_REQ * runProgress.wave;

    if (stats.kills >= waveReq && (currentState === 'PHASE_1' || currentState === 'LEVEL_UP')) {
