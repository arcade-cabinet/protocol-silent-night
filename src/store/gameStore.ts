...
export const useGameStore = create<GameStore>((set, get) => ({
  ...
  selectLevelUpgrade: (upgradeId) => {
    ...
    set({
      ...
      playerHp: newHp,
      lastLevelUpExitTime: Date.now(), // Track when we exit LEVEL_UP state
    });
  ...
  damageBoss: (amount) => {
    ...
    if (newHp <= 0) {
      ...
      // Check if this is the first boss (wave 1) - if so, transition to WIN state
      // Otherwise, continue to endless mode
      if (runProgress.wave === 1) {
        // First boss defeated - WIN!
        set({
          state: 'WIN',
          bossActive: false,
