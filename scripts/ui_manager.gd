...
func _build_present_buttons(present_defs: Dictionary, save_manager: Node, on_class_pressed: Callable) -> void:
	...
	for present_id in present_defs.keys():
		...
		var button := Button.new()
		var unlocked := _is_present_unlocked(def, best_wave)
		var label: String = "%s\n%s" % [def.get("name", present_id), def.get("tagline", "")]
		if not unlocked:
			label += "\n[LOCKED: %s]" % _unlock_label(def.get("unlock", ""))
		button.text = label
		button.custom_minimum_size = Vector2(200, 140)
...
func _is_present_unlocked(def: Dictionary, best_wave: int) -> bool:
