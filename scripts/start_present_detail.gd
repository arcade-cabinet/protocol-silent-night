...
static func build(parent: BoxContainer, layout: Dictionary) -> Dictionary:
	...
	shell.custom_minimum_size = Vector2(maxf(260.0, safe_size.x - 48.0), 0.0) if stacked_mobile else Vector2(clampf(safe_size.x * 0.28, 260.0, 340.0), 0.0)
	...
	preview_host.custom_minimum_size = Vector2(0.0, 148.0 if is_mobile else 196.0)
	...
	stats_label.text = "DMG --  RATE --  RNG --  PIERCE --"
