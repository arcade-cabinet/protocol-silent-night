...
static func behavior_swerve(enemy: Dictionary, player_node: Node3D, delta: float, on_move_actor: Callable) -> void:
	...
	var offset := sin(phase) * 0.9
	var direction: Vector3 = (forward + side * offset).normalized()
	on_move_actor.call(enemy["node"], direction, float(enemy["speed"]), delta, contact_radius(enemy))


static func behavior_stomp(enemy: Dictionary, player_node: Node3D, delta: float, on_move_actor: Callable) -> void:
	enemy["behavior_timer"] = float(enemy.get("behavior_timer", 0.0)) + delta
	...
	match state:
		...
		"windup":
			if float(enemy["behavior_timer"]) >= 0.4:
				...
				enemy["behavior_timer"] = 0.0
		"surge":
			on_move_actor.call(enemy["node"], direction, speed * 2.0, delta, contact_radius(enemy))
...
static func behavior_flank(enemy: Dictionary, player_node: Node3D, delta: float, on_move_actor: Callable, on_spawn_projectile: Callable) -> void:
	enemy["behavior_timer"] = float(enemy.get("behavior_timer", 0.0)) + delta
	var to_player: Vector3 = player_node.position - enemy["node"].position
	to_player.y = 0.0
	...
	var radial_bias := 0.0
	if distance > orbit_radius + 0.5:
		radial_bias = 0.7
	elif distance < orbit_radius - 0.5:
		radial_bias = -0.7
	var direction: Vector3 = (tangent + forward * radial_bias).normalized()
	on_move_actor.call(enemy["node"], direction, float(enemy["speed"]), delta, contact_radius(enemy))
	if float(enemy["behavior_timer"]) >= 1.2:
		enemy["behavior_timer"] = 0.0
		on_spawn_projectile.call(enemy["node"].position + Vector3(0, 0.5, 0), forward, true, 6.0, 1, 14.0, 0.28)


static func behavior_ranged(enemy: Dictionary, player_node: Node3D, delta: float, on_move_actor: Callable, on_spawn_projectile: Callable) -> void:
	enemy["behavior_timer"] = float(enemy.get("behavior_timer", 0.0)) + delta
	var to_player: Vector3 = player_node.position - enemy["node"].position
	to_player.y = 0.0
	var distance := to_player.length()
	...
	if float(enemy["behavior_timer"]) >= 2.0:
		enemy["behavior_timer"] = 0.0
...
static func behavior_pack(enemy: Dictionary, enemies: Array, player_node: Node3D, delta: float, on_move_actor: Callable) -> void:
	var center: Vector3 = enemy["node"].position
	var count := 1
	for other in enemies:
		if other.get("id", "") != "bumble" or other.get("enemy_uid", 0) == enemy.get("enemy_uid", -1):
			continue
		if other["node"].position.distance_to(enemy["node"].position) < 6.0:
