...
static func behavior_flank(enemy: Dictionary, player_node: Node3D, delta: float, on_move_actor: Callable, on_spawn_projectile: Callable, phase_level: int = 1) -> void:
	enemy["behavior_timer"] = float(enemy.get("behavior_timer", 0.0)) + delta
	var to_player: Vector3 = player_node.position - enemy["node"].position
	to_player.y = 0.0
...
static func behavior_ranged(enemy: Dictionary, player_node: Node3D, delta: float, on_move_actor: Callable, on_spawn_projectile: Callable, phase_level: int = 1) -> void:
	enemy["behavior_timer"] = float(enemy.get("behavior_timer", 0.0)) + delta
	var to_player: Vector3 = player_node.position - enemy["node"].position
	to_player.y = 0.0
	var distance := to_player.length()
