...
func test_build_board_foundation_adds_surface_and_border_nodes() -> void:
	...
	# outer_field + arena_surface + 4 border walls = 6
	assert_int(_root.get_child_count()).is_equal(6)
...
func test_build_snow_drifts_adds_one_node_per_drift() -> void:
	...
	_builder.build_snow_drifts(_root, board_data)
	assert_int(_root.get_child_count()).is_equal(3)


func test_build_snow_drifts_empty_board_adds_nothing() -> void:
	_builder.build_snow_drifts(_root, {})
	assert_int(_root.get_child_count()).is_equal(0)


func test_build_outer_ridge_adds_24_chunks() -> void:
	# 4 perimeter segments × 6 steps each = 24 ice-chunk nodes.
	_builder.build_outer_ridge(_root, {})
	assert_int(_root.get_child_count()).is_equal(24)
...
func test_build_outer_ridge_is_deterministic() -> void:
	...
	_builder.build_outer_ridge(root_a, {})
	_builder.build_outer_ridge(root_b, {})
