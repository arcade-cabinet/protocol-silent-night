...
func test_mobile_feedback_enabled_by_default_on_portrait_layout() -> void:
	assert_bool(MOBILE_FEEDBACK.enabled_for_viewport(Vector2(390.0, 844.0))).is_true()
...
func test_mobile_feedback_respects_saved_toggle() -> void:
	...
	assert_bool(MOBILE_FEEDBACK.enabled_for_viewport(Vector2(390.0, 844.0), save)).is_false()


func test_mobile_feedback_note_text_reports_disabled_state() -> void:
	var save: SaveDouble = auto_free(SaveDouble.new())
	save.set_preference("mobile_haptics", false)
	assert_str(MOBILE_FEEDBACK.note_text(Vector2(390.0, 844.0), save)).is_equal("Haptics OFF")
