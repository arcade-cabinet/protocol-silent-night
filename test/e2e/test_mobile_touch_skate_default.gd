...
func _initialize() -> void:
	...
	_save_manager.set_save_path_for_tests("user://e2e_mobile_touch_skate_default.json")
...
func _run() -> void:
	...
	var present_button := _present_button("p_01")
	_assert(present_button != null, "Expected Candy Warden present button")
	_assert(bool(present_button.get_meta("unlocked", false)), "Expected Candy Warden to be unlocked for the skate run")
	await _activate_button(present_button, func() -> bool: return _main.current_class_id == "p_01")
	...
	_assert(String(profile["handedness"]) == "right", "Expected right-handed touch profile")
	_assert(String(profile["dash_label"]) == "SLIDE", "Expected skate dash verb")
	_assert(dash_rect.end.x > float(WINDOW_SIZE.x) - 40.0, "Expected right-handed dash zone to live on the right edge")
	_assert(_main.ui_mgr.dash_button.text == "SLIDE", "Expected live dash button text to match skate doctrine")
	_main.debug_spawn_boss()
	await _step_until_target_hint("HUNT LOCK", 16)

	var move_origin := Vector2(96.0, dash_rect.position.y - 180.0)
	...
	_assert(_main.ui_mgr.joystick_base.visible, "Expected joystick to appear during skate move drag")
	_assert(_main.player_node.global_position.distance_to(start_pos) > 0.75, "Expected player movement under skate drag")

	var dash_center := dash_rect.get_center()
	_game_touch(dash_center, 1, true)
	await _step(1)
	_assert(_main.dash_cooldown_timer > 0.0, "Expected right-edge dash touch to trigger cooldown")
	_assert(_main.dash_timer > 0.0, "Expected right-edge dash touch to trigger movement")
	_assert(_main.touch_active, "Expected move touch to survive right-edge dash touch")
