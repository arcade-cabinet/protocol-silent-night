...
func _initialize() -> void:
	...
	_save_manager.set_save_path_for_tests("user://e2e_mobile_touch_sweep_unlock.json")
	_save_manager.reset_state_for_tests()
	_save_manager.load_state()
	_save_manager.register_wave_reached(3)
...
func _run() -> void:
	...
	await _activate_button(present_button, func() -> bool: return _main.current_class_id == "p_06")
	...
	_assert(String(profile["dash_label"]) == "JINK", "Expected sweep dash verb")
	_assert(dash_rect.end.x > float(WINDOW_SIZE.x) - 40.0, "Expected right-handed dash zone to live on the right edge")
	_assert(_main.ui_mgr.dash_button.text == "JINK", "Expected live dash button text to match sweep doctrine")
	_main.debug_spawn_boss()
	await _step_until_target_hint("SWEEP LOCK", 16)
	...
	_assert(_main.ui_mgr.joystick_base.visible, "Expected joystick to appear during sweep move drag")
	_assert(_main.player_node.global_position.distance_to(start_pos) > 0.75, "Expected player movement under sweep drag")
