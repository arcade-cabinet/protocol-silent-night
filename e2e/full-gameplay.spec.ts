...
// Increase timeout for full gameplay tests
test.setTimeout(120000);

// Helper to click COMMENCE OPERATION button reliably
async function clickCommenceOperation(page: Page, timeout = 30000) {
...
test.describe('Full Gameplay - MECHA-SANTA (Tank Class)', () => {
  test('should complete full game loop with Santa', async ({ page }) => {
    ...
    await page.waitForTimeout(500); // Stabilize UI
...
test.describe('Full Gameplay - Game Reset', () => {
  ...
  test('should preserve high score after reset', async ({ page }) => {
    ...
    await commenceBtn2.waitFor({ state: 'visible', timeout: 20000 });
    await commenceBtn2.click();
