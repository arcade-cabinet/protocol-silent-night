...
test.describe('Full Gameplay - MECHA-SANTA (Tank Class)', () => {
  ...
  test('should survive longer due to high HP', async ({ page }) => {
    ...
    // Clear any enemies that may have spawned and dealt damage
    await page.evaluate(() => {
      const store = (window as any).useGameStore;
      if (store) {
        const state = store.getState();
        state.updateEnemies(() => []);
        // Reset HP to expected value for MECHA-SANTA (300)
        store.setState({ playerHp: 300 });
      }
    });
    await page.waitForTimeout(100);
  ...
  test('should have balanced survivability', async ({ page }) => {
    ...
    await page.evaluate(() => {
      ...
      if (store) {
        const state = store.getState();
        state.updateEnemies(() => []);
        // Reset HP to expected value for Bumble (200)
        store.setState({ playerHp: 200 });
      }
    });
    await page.waitForTimeout(100);
