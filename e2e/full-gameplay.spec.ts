...
test.describe('Full Gameplay - MECHA-SANTA (Tank Class)', () => {
  test('should complete full game loop with Santa', async ({ page }) => {
    ...
    expect(state?.playerHp).toBeGreaterThanOrEqual(299); // Allow for timing variations
  ...
  test('should survive longer due to high HP', async ({ page }) => {
    test.setTimeout(180000); // Increase timeout to 3 minutes for this test
    await setupPage(page);
...
test.describe('Full Gameplay - Boss Battle', () => {
  ...
  test('should defeat boss and win game', async ({ page }) => {
    ...
    // Trigger boss spawn with faster delays
    for (let i = 0; i < 10; i++) {
      await triggerStoreAction(page, 'addKill', 10);
      await page.waitForTimeout(100);
      await handleAllLevelUps(page);
    ...
    // Verify victory screen - use heading role for specificity with longer timeout
...
test.describe('Full Gameplay - Kill Streaks', () => {
  test('should trigger kill streak notifications', async ({ page }) => {
    ...
    // Rapid kills to build streak - keep delays short for streak
    await triggerStoreAction(page, 'addKill', 10);
    await page.waitForTimeout(100);
    await triggerStoreAction(page, 'addKill', 10);
    await page.waitForTimeout(100);
  ...
  test('should reset streak after timeout', async ({ page }) => {
    ...
    // Build a streak with faster delays
    await triggerStoreAction(page, 'addKill', 10);
    await page.waitForTimeout(100);
    await triggerStoreAction(page, 'addKill', 10);
    await page.waitForTimeout(100);
  ...
  test('should apply streak bonus to score', async ({ page }) => {
    ...
    // Second kill - 25% bonus (streak of 2) - keep kills close together for streak
    await triggerStoreAction(page, 'addKill', 100);
    await page.waitForTimeout(50);
    ...
    // Third kill - 50% bonus (streak of 3) - keep kills close together for streak
    await triggerStoreAction(page, 'addKill', 100);
    await page.waitForTimeout(50);
...
test.describe('Full Gameplay - Game Reset', () => {
  ...
  test('should preserve high score after reset', async ({ page }) => {
    ...
    // Accumulate kills faster with shorter delays
    for (let i = 0; i < 5; i++) {
      await triggerStoreAction(page, 'addKill', 100);
      await page.waitForTimeout(100);
      await handleAllLevelUps(page);
    ...
    // Wait for game over screen and reset
    await expect(page.getByRole('button', { name: /RE-DEPLOY/ })).toBeVisible({ timeout: 10000 });
    await safeClick(page, page.getByRole('button', { name: /RE-DEPLOY/ }));
    await page.waitForTimeout(2000);
...
test.describe('Full Gameplay - Complete Playthrough', () => {
  test('should complete entire game as Santa', async ({ page }) => {
    ...
    // Step 3: Combat phase - kill enemies with faster delays
    for (let i = 0; i < 10; i++) {
      await triggerStoreAction(page, 'addKill', 10);
      await page.waitForTimeout(100);
      await handleAllLevelUps(page);
    ...
    // Step 4: Boss phase - ensure level-ups are handled
    await page.waitForTimeout(1000);
    await handleAllLevelUps(page);
    state = await getGameState(page);
    expect(state?.gameState).toBe('PHASE_BOSS');
    await expect(page.getByText('⚠ KRAMPUS-PRIME ⚠')).toBeVisible({ timeout: 10000 });
    ...
    // Step 7: Can restart - wait for button to be visible first
    await expect(page.getByRole('button', { name: /RE-DEPLOY/ })).toBeVisible({ timeout: 10000 });
    await safeClick(page, page.getByRole('button', { name: /RE-DEPLOY/ }));
    await page.waitForTimeout(2000);
