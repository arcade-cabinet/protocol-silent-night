...
test.describe('Full Gameplay - Kill Streaks', () => {
  test('should trigger kill streak notifications', async ({ page }) => {
    ...
    // Rapid kills to build streak - execute synchronously to ensure timing
    await page.evaluate(() => {
      const store = (window as any).useGameStore;
      if (!store) return;
      const state = store.getState();

      // Execute kills in rapid succession to build streak
      state.addKill(10);
      state.addKill(10);
      state.addKill(10);
    });

    // Wait for state to settle and UI to update
    await page.waitForTimeout(1000);
