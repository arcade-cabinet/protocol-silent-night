...
test.describe('Full Gameplay - MECHA-SANTA (Tank Class)', () => {
  ...
  test('should survive longer due to high HP', async ({ page }) => {
    ...
    let state = await getGameState(page);
    // Allow small variance due to enemy collision damage during game loop
    expect(state?.playerHp).toBeGreaterThanOrEqual(190); // ~200 - some collision damage
    expect(state?.playerHp).toBeLessThanOrEqual(200); // 300 - 100 = 200
    ...
    state = await getGameState(page);
    expect(state?.playerHp).toBeGreaterThanOrEqual(90); // ~100 - some collision damage
...
test.describe('Full Gameplay - Kill Streaks', () => {
  test('should trigger kill streak notifications', async ({ page }) => {
    ...
    // Rapid kills to build streak - must be within 2000ms to maintain streak
    await triggerStoreAction(page, 'addKill', 10);
    await page.waitForTimeout(100); // Small delay for state to update
    await triggerStoreAction(page, 'addKill', 10);
    await page.waitForTimeout(300); // Wait for state propagation
  ...
  test('should apply streak bonus to score', async ({ page }) => {
    ...
    // Must be within 2000ms of first kill to maintain streak
    await triggerStoreAction(page, 'addKill', 100);
    await page.waitForTimeout(300); // Wait for state propagation
